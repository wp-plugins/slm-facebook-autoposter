...
/**
 * Classes have been renamed to avoid conflicts in case classes with the same name exist there
 * All the original code and logic except of class names has been preserverd
 */
 ...
 */
class MV_FacebookApiException extends Exception
{
  /**
 ...
 * @author Naitik Shah <naitik@facebook.com>
 */
abstract class MV_BaseFacebook
{
  /**
   * Version.
   */
   ...
   * The cookie is not set by the MV_BaseFacebook class, but
   ...
   * metadata. The cookie is not set by the MV_BaseFacebook class, but it may be
   ...
   */
  protected function getAccessTokenFromCode($code, $redirect_uri = null, $bul=true) {
  	//echo "getAccessTokenFromCode 1<br/>";
    if (empty($code)) {
      return false;
    }

  	//echo "getAccessTokenFromCode 2<br/>";
    ...
    try {
      // need to circumvent json_decode by calling _oauthRequest
      // directly, since response isn't JSON format.
  	//echo "getAccessTokenFromCode 3<br/>";
        ...
        $this->_oauthRequest(
          ...
          $params = array('client_id' => $this->getAppId(),
                          ...
                          'code' => $code),$bul);
      //echo 'ACCESS_TOKEN RESP<br/>';
      //echo'<pre>';print_r($access_token_response);echo'</pre>';
    } catch (MV_FacebookApiException $e) {
      // most likely that user very recently revoked authorization.
      // In any event, we don't have an access token, so say so.
      return false;
    ...
    //echo "HERE-><br/>";
    //echo'<pre>';print_r($response_params);echo'</pre>';
   ...
   * @throws MV_FacebookApiException
   */
   ...
   * @throws MV_FacebookApiException
   */
  protected function _oauthRequest($url, $params, $bul=true) {
    if (!isset($params['access_token']) && $bul) {
  ...
  protected function makeRequest($url, $params, $ch=null) {
    ...
    if ($result === false) {
      $e = new MV_FacebookApiException(array(
  ...
  protected function throwAPIException($result) {
    $e = new MV_FacebookApiException($result);
    switch ($e->getType()) {
      // OAuth 2.0 Draft 00 style
      case 'OAuthException':
        // OAuth 2.0 Draft 10 style
  ...
  /**
   * Get the data for $key, persisted by MV_BaseFacebook::setPersistentData()
   *
